...
interface DownloadLongestButtonProps {
  playlist: PlaylistData;
}

export default function DownloadLongestButton({
  playlist,
}: DownloadLongestButtonProps) {
  const [isDownloading, setIsDownloading] = useState(false);
  const [progress, setProgress] = useState(0);

  const downloadLongestTracks = async () => {
    ...
    try {
      ...
      playlist.tracks.forEach((trackSet) => {
        trackSet.tracks.forEach((track) => {
          // Ignora faixas sem link para evitar requisições inválidas que quebram o ZIP
      ...
      const analyzePromises = uniqueTitles.map(async (title, index) => {
        ...
        let longestVersion = versions[0];
        let maxSize = -Infinity; // Começa baixo para encontrar o maior valor
        ...
        chosenTracks.push({
          title,
          setLabel: longestVersion.setLabel,
          src: longestVersion.src,
        });
      ...
      const sanitizedName = playlist.name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-");
      link.download = `${sanitizedName}-longas.zip`;
  ...
  return (
    <button
      type="button"
      onClick={downloadLongestTracks}
      disabled={isDownloading}
      className="flex items-center gap-1 rounded border border-[#3c2520]/60 bg-[#1f1512] px-2 py-0.5 font-medium text-[#8a7570] text-[9px] hover:border-[#5a362e] hover:bg-[#2c1d19] hover:text-[#f3eae8] disabled:opacity-40"
      title="Baixar todas as faixas desta playlist em ZIP (as versões mais longas)"
    >
      <Icon
        icon={
          isDownloading ? "svg-spinners:180-ring" : "ph:hourglass-high-bold"
        }
        width={12}
      />
